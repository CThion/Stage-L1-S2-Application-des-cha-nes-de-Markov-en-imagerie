...
#recuitSimule_Vstar("losange", 64, 1, 0, 0.6, 2*10^4, 0.2, 5)

```

```{r graph_Vstar}
MCdeMC_revo=function(forme, N, p, beta, B, alpha, n, k){
  corresp = c(1,k)
  longueur = c(1:k)
  for( L in(1:k)){ #
    corresp[L] = recuitSimule_Vstar(forme, N, beta, B, alpha, n, p, L)
  }#for
...
MCdeMC_revo("rectangle", 64, 0.25, 0.66, 0, 0.33, 2*10^4, 10)
```
...
```{r smartStopLight}
smartStopLight=function(forme, N, beta, B, alpha, n, m, p){
	#fonction qui réalise l'algorithme de Gibbs sur une image bruitée img1 (à partir de img0),pour restaurer cette image.
...
#smartStopLight("rectangle", 64, 2/3, 0, 1/3, 500, 15, 0.25)
```
