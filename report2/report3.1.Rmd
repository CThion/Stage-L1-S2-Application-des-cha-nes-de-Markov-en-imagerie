...
L'objectif final de ce stage est de restaurer algorithmiquement des binaires (assez simples) en noir et blanc ayant subi un bruitage. Pour cela on va utiliser les chaînes de Markov et des modèles énergétiques sur les images, principalement le modèle d'Ising et la mesure de Gibbs, afin de pouvoir estimer de manière probabiliste les meilleurs configurations pour la restauration.  

On utilise les notations et appellations suivantes:  
- x : une configuration de la grille des pixels (une image donc), parmis l'ensemble des configurations possibles $\Omega$. On note X la variable aléatoire décrivant x.   
...
A priori pour "deviner" quelle est la bonne valeur pour un pixel s dans une image donnée, le mieux à faire est de regarder tous le reste de l'image et essayer d'exprimer $X_s$.   

On fait cependant l'hypothèse que connaître $V_s$ le voisinage proche de s est équivalent à connaître tous le reste de l'image, soit $P(X_s=x_s|\tilde X^s)=P(X_s=x_s|V_s)$. La définition du voisinage proche peut varier, par son étendu et sa forme; le voisinage que nous utiliserons presque toujours est simplement l'ensemble des quatre plus proches voisins de s sur la verticale et l'horizontale. On ne considère pas les diagonales.
...
On verra une de ses lqimites dans la partie sur l'estimation de $\beta$.
...
Deux algorithmes existes pour simuler un champ de Markov.
...
C'est ce que l'on fait dans l'algorithme de metropolis suivant:
