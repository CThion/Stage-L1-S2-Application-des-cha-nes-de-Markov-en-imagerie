...
> **Comparaison de performance** entre l'algorithme de recuit et celui de Metropolis. On ajoute la fonction de correspondance au deux pour comparer numériquement. On constate que la difference est quasiment nulle.
...
- Le terme concernant le voisinage dans le calcul de U est maintenant cindé en deux, avec deux $\beta$ différents, $\betah$ pour l'horizontale et $\betav$ pour la verticale. On a ainsi: $U(x_s)= -x_s[\beta_h(x_d+x_g)+\beta_v(x_h+x_b)+\alpha x_s^0]$ avec $x_d,x_g,x_h,x_b$ les valeurs des pixels voisins du site s respectivement à droite, gauche, haut, bas.
