...
Le parametre alpha détermine le poid donné à l'image bruité initiale. Si alpha est trop grand, les valeurs les plus probables seront toujours celles de l'image bruitée, et donc l'algorithme ne fera rien. Si il est trop petit voir nulle, la configuration initiale va être oubliée petit à petit, et on va tendre vers l'image branche, soit donc une la configuration d'énergie minimale ne tenant pas compte de l'image bruitée.  


```{r}
#========================================================================================================

metropolisIsing=function(forme, N, beta, B, alpha, n){
...
metropolisIsing("rectangle", 64, 1, 0, 1, 10^5)
...
GibbsIsing=function(forme, N, beta, B, alpha, n){
  #fonction qui réalise l'algorithme de Metropolis sur une image img0 qui est bruité en img1, et doit à la fin être restaurée en img2
...
GibbsIsing("rectangle", 64, 2, 0, 0.5, 10^4)
#========================================================================================================
```
...
recuitMetro=function(forme, N, beta, B, n){
  #fonction qui réalise l'algorithme de Metropolis sur une image img0 qui est bruité en img1, et doit à la fin être restaurée en img2
  ...
  for(k in (1:n)){
    ...
    iaux <- 2 + k%%N #de 2 à N+1
    jaux <- 2 + k%/%(N-1)%%N #de 2 à N+1
    #----on test l'autre valeur possible sur l'image tampon. Si c'est ok on fait le changement sur img1 aussi.----
    img2[i,j] = -img1[i,j]
    #----voisinage----
    v = img1[i+1,j] + img1[i-1,j] + img1[i,j+1] + img1[i,j-1]
    #----potentiels----
    U1 = -B*img1[i,j]/T-beta*img1[i,j]*v/T #potentiel actuel
    U2 = -B*img2[i,j]/T-beta*img2[i,j]*v/T #potentiel candidat
    #----test----
    dU = U2-U1
    if(dU<0){img1[i,j]= -img1[i,j]} #si U1 > U2, valeur candidate acceptée
    else{
      p <- exp(-dU)
      if(runif(1)<=p){img1[i,j]= -img1[i,j]} #valeur candidate acceptée selon sa proba conditionnelle
      else{img2[i,j] <- img1[i,j]} #sinon on ne change pas
    }#else
  }#for
  #----affichage----
...
recuitMetro("rectangle", 64, 1, 1, 10^4)
#========================================================================================================
```
...
Vbranch2=function(img, N, is, js, ib, jb){
  #fonction qui forme UNE branche de voisinage de s (is, js), avec les coordonnées du lead qui sont (ib, jb)
  
  #----VARIABLES----
  dii <- ib-is #coord ligne 
  djj <- jb-js#coord colonnes
  lead <- img[ib,jb]
  branch <- lead #la branche est de longeur minimale 1 (cas du voisinage standard)
  cdd = 0 #candidat
  k <- 1
  ...
  if(dii==0){
    ...
    #--orientée gauche--
    if(djj<0){
      cdd <- img[ib,jb-k] #avec la bordure de 0 auxiliaire, on ne devrait pas avoir d'erreure
      print(cdd)
      print(lead)
      if(cdd==lead){
        while(cdd==lead){
...
```{r}
Vstar=function(img, N, i,j){
  s <- 0
  s <- s + Vbranch2(img, N, i, j, i-1, j)
  s <- s + Vbranch2(img, N, i, j, i+1, j)
  s <- s + Vbranch2(img, N, i, j, i, j-1)
  s <- s + Vbranch2(img, N, i, j, i, j+1)
  return(s)
}#func
...
```{r}
recuitSimule(400,2/3,64,0.2,0.3,0)
```
...
MCdeMC_revo=function(n, beta, N, p, alpha, stop){
  ...
  ggplot(data = rev, aes(x="nombre_de_revolution", y="pourcentage de correspondance")) + geom_point() + geom_smooth() #+ geom_line()
}#func
